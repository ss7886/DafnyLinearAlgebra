...
// function norm(vec : Vector) : (norm : real)
// reads *
// ensures norm == dot_product(vec, vec)
// {
//     norm_aux(vec, 0)
// }

// function norm_aux(vec : Vector, i : int) : (norm_aux : real)
// reads *
// requires 0 <= i <= vecLength(vec)
// decreases vecLength(vec) - i
// ensures norm_aux == dot_product_aux(vec, vec, i)
// {
//     if i == vecLength(vec)
//     then 0.0
//     else vecGet(vec, i) * vecGet(vec, i) + norm_aux(vec, i + 1)
// }

// method dot_product_loop(vec1 : Vector, vec2 : Vector) returns (sum : real)
// requires vecLength(vec1) == vecLength(vec2)
// ensures sum == dot_product(vec1, vec2)
// {
//     sum := 0.0;
//     var i := vecLength(vec1);
//     while (i > 0)
//     invariant 0 <= i
//     invariant sum == dot_product_aux(vec1, vec2, i)
//     {
//         sum := vecGet(vec1, i - 1) * vecGet(vec2, i - 1) + sum;
//         i := i - 1;
//     }
// }